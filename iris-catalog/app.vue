...
// Dynamic title + favicon reflecting fleet status
const fleetSummary = computed(() => {
    const entries = Object.values(status.value);
    if (entries.length === 0) {
        return { title: "Iris Catalog", color: "#7A8478" };
    }
    const down = entries.filter(s => s.aggregated === "down").length;
    const degraded = entries.filter(s => s.aggregated === "degraded").length;
    if (down > 0) {
        return { title: `Iris — ${down} DOWN`, color: "#E67E80" };
    }
    if (degraded > 0) {
        return { title: `Iris — ${degraded} degraded`, color: "#DBBC7F" };
    }
    return { title: "Iris — All Clear", color: "#A7C080" };
});
